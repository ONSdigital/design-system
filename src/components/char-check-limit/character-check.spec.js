...
describe('script: character-check', () => {
  ...
  describe('Given that the input value is cleared programatically', () => {
    beforeEach(async () => {
      await page.type('#search-field', '1', { delay: 20 });
      await page.$eval('#search-field', element => (element.value = ''));
      await page.focus('#search-field');
      await page.keyboard.press('Backspace');
    });

    it('then aria-live attribute should not be added', async () => {
      const hasAriaLiveAttribute = await page.$eval('#search-field-check-remaining', element => element.hasAttribute('aria-live'));
