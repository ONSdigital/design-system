...
describe('script: autosuggest', () => {
    ...
    describe('GIVEN: Params: None', () => {
        describe('WHEN: the component initialises', () => {
            test('THEN: the input should be given the correct aria attributes', async () => {
            ...
            test('THEN: the autocomplete attribute be set to be not set to on', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: the instructions, listbox, and status should become visible', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));

                const instructionsDisplayStyle = await page.$eval(
                    '.ons-js-autosuggest-instructions',
        ...
        describe('WHEN: the user presses the "Down" arrow key', () => {
            test('THEN: it navigates to the first suggestion initially', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it navigates to the suggestion below the current suggestion', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it marks suggestion as being selected', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
                ...
                const ariaSelectedValue = await page.$eval('.ons-autosuggest__option--focused', (node) =>
            ...
            test('THEN: it sets aria status to a message showing the selected result', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it does not mark other suggestions as being selected', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it does not affect suggestion selection when already on last item', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it does not interfere with text input', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
        ...
        describe('WHEN: the user presses the "Up" arrow key', () => {
            ...
            test('THEN: it navigates to the suggestion above the current suggestion', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it does not affect suggestion selection when already on first item', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
        ...
        describe('WHEN: the user presses the "Enter" key', () => {
            test('THEN: it accepts the selected suggestion', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
        ...
        describe('WHEN: the user blurs the input', () => {
            test('THEN: suggestions should remain visible for 300ms', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it clears innerHTML of listbox', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it clears `has-results` modifier of autosuggest component', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it clears aria attributes of input element', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
        ...
        describe('WHEN: the user clicks on a result', () => {
            test('THEN: accepts the suggestion', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
        ...
        describe('WHEN: the user inputs text', () => {
            test('THEN: it does not show suggestions when input length < minimum characters', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it shows suggestions when input length >= minimum characters', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it gets the language if set', async () => {
        ...
        describe('WHEN: the mouse moves over a result and a suggestion is focused', () => {
            test('THEN: it removes the focused class', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
        ...
        describe('WHEN: the mouse moves off a result and a suggestion was focused', () => {
            test('THEN: it restores the focused class', async () => {
                await setTestPage(
                    '/test',
                    `
        ...
        describe('WHEN: there are results', () => {
            test('THEN: it un-highlights a previously highlighted suggestion', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it decorates input element with `aria-expanded` attribute', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it emboldens matched suggestion text', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it does not embolden anything when suggestion does not contain query text', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it sets aria status to a message asking for more characters to be input', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it sets aria status to a message indicating a count of one suggestion', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it sets aria status to a message indicating the count of suggestions', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
    ...
    describe('GIVEN: there are no results', () => {
        describe('WHEN: `noResults` content is provided', () => {
            test('THEN: it outputs `noResults` content', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it sets aria status to a message indicating that query is too short for suggestions', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
            ...
            test('THEN: it sets aria status to a message indicating the zero count of suggestions', async () => {
                await setTestPage('/test', renderComponent('autosuggest', EXAMPLE_AUTOSUGGEST));
        ...
        describe('WHEN: `noResults` content is not provided', () => {
            test('THEN: it has an empty listbox', async () => {
                await setTestPage(
                    '/test',
    ...
    describe('GIVEN: there are no results due to an error', () => {
        describe('WHEN:  the status code is 400', () => {
            ...
            test('THEN: it shows the type more message', async () => {
        ...
        describe.each([
            ...
            test('THEN: it shows the API error message', async () => {
            ...
            test('THEN: the list and results element should be removed from the page', async () => {
            ...
            test('THEN: the input should be disabled', async () => {
            ...
            test('THEN: the input value should be empty', async () => {
                const inputValue = await page.$eval('.ons-js-autosuggest-input', (node) => node.value);
                expect(inputValue).toBe('');
            });

            test('THEN: the label class should be added', async () => {
                const hasClass = await page.$eval('.ons-label', (node) => node.classList.contains('ons-u-lighter'));
                expect(hasClass).toBe(true);
            });

            test('THEN: the aria status should be set', async () => {
                const statusMessage = await page.$eval('.ons-js-autosuggest-aria-status', (node) => node.textContent);
    ...
    describe('GIVEN: the component initialises with the allowMultiple parameter', () => {
        describe('WHEN: a result is selected', () => {
            test('THEN: the input value should contain a comma when focused', async () => {
                await setTestPage(
                    '/test',
                    renderComponent('autosuggest', {
                        ...EXAMPLE_AUTOSUGGEST,
        ...
        describe('WHEN: the user blurs the input', () => {
            test('THEN: the input value should not contain a comma', async () => {
                await setTestPage(
                    '/test',
                    renderComponent('autosuggest', {
                        ...EXAMPLE_AUTOSUGGEST,
