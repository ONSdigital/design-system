...
### Run tests in SauceLabs

All unit tests are automatically cross browser tested in [SauceLabs](https://saucelabs.com) by [TravisCI](https://travis-ci.org/ONSdigital/design-system) when a pull request is created or if a change is pushed to an existing pull request.

You can also run cross browser testing in SauceLabs manually against your local branch by running this command:

_Note_: You will need to set your `SAUCE_USERNAME` and `SAUCE_ACCESS_KEY` environment variables to allow authentication with SauceLabs.
