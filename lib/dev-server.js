...
(async () => {
    ...
    app.get('/:type(components|patterns|foundations)', async (req, res) => {
        ...
        const output = await handleTypeRoute(sanitize(type), null, res);
        res.send(output);
    });
    ...
    app.get('/:type(components|patterns|foundations)/:name', async (req, res) => {
        ...
        const output = await handleTypeRoute(sanitize(type), sanitize(name), res);
        res.send(output);
    });
    ...
    app.get('/:type(components|patterns|foundations)/:name/:exampleName', async (req, res) => {
        ...
        const output = await handleExamplesRoute(sanitize(type), sanitize(name), sanitize(exampleName));
        res.send(output);
    });
