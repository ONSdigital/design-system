...
function buildSiteMap(context) {
  let templates = getAllTemplates(context).filter(path => !path.includes('/examples/'));
  templates = templates.filter(path => !path.includes('/_documentation-template/'));
  const siteMap = [];
...
function getPageInfo(context) {
  ...
  if (pageRef) {
    ...
    return { url: path, siteMap, children, parent, title, experimental };
  } else {
    return { url: path };
  }
}
