...
const newComponentsPath = `${templatesPath}/components`;
const layoutsPath = `${sourcePath}/styles/page-template`;
const newLayoutsPath = `${templatesPath}/layouts`;
...
async function removeExistingFolders() {
  const folders = [newComponentsPath, newLayoutsPath, ...assetFolders];
...
async function copyTemplates() {
  await createFolder(newLayoutsPath);

  try {
    const items = await fs.readdirSync(layoutsPath).filter(path => path.includes('.njk'));
...
async function copyTemplate(templateFileName) {
  try {
    await copyFile(`${layoutsPath}/${templateFileName}`, `${newLayoutsPath}/${templateFileName}`);
  } catch (error) {
    throw new Error(error);
  }
}
...
async function run() {
  ...
  await createFolder(templatesPath);
